...
# (chulayuth) Copied options from SD15 inferencing code (so that training code can do the same)
_CLIP_SKIP = None
...
def parse_args():
    ...
    parser.add_argument("--pretrained_model_name_or_path",type=str,default="stable-diffusion-v1-5/stable-diffusion-inpainting",required=False,help="Path to pretrained model or model identifier from huggingface.co/models.",)
    parser.add_argument("--pretrained_garmentnet_path",type=str,default="stable-diffusion-v1-5/stable-diffusion-v1-5",required=False,help="Path to pretrained model or model identifier from huggingface.co/models.",)

    parser.add_argument("--logging_epoch", type=int, default=10, help="Loss/Metrics logging frequency.",)
    parser.add_argument("--logging_dir",type=str,default="logs_sd15",help="The output directory where tensorboard log will be written.",)
    ...
    parser.add_argument("--pretrained_ip_adapter_path",type=str,default="ckpt/ip_adapter/ip-adapter-plus_sd15.bin",help="Path to pretrained ip adapter model. If not specified weights are initialized randomly.",)
    ...
    parser.add_argument("--output_dir",type=str,default="output_sd15",help="The output directory where the model predictions and checkpoints will be written.",)
...
def main():
    ...
    _debug_print(f'>>>>>>>>>>>>>>>>>>> args.pretrained_model_name_or_path = {args.pretrained_model_name_or_path}')
    ...
    image_proj_model = Resampler(
        dim=768, # image_encoder.config.hidden_size,
        depth=4,
        dim_head=64,
        heads=12, # 20,
    ...
    conv_new = torch.nn.Conv2d(
        in_channels=4+4+1+4,  # Needs to be 13 channels instead of 9.
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet), accelerator.accumulate(image_proj_model):
                    ...
                    if accelerator.is_main_process:
                        with torch.no_grad():
                            with torch.cuda.amp.autocast():
                                ...
                                print_gpu_mem('Step 8 - Before loading newpipe')
                                newpipe = TryonPipeline.from_pretrained(
                                    ...
                                    scheduler=noise_scheduler,
                                    tokenizer=tokenizer,
                                    tokenizer_2=tokenizer,  # SD15 does not have "2"
                                    text_encoder=text_encoder,
                                    text_encoder_2=text_encoder, # SD15 does not have "2"
                                    image_encoder=image_encoder,
                                    unet_encoder = unet_encoder,
                                    torch_dtype=torch.float16,
                                    add_watermarker=False,
                                ...
                                print_gpu_mem('Step 9 - After loading newpipe')
                                _debug_print(f'>>>>>>>>>>>>>>>>>>> newpipe.device = {newpipe.device}')
                                with torch.no_grad():
                                    for sample in test_dataloader:
                                        ...
                                        with torch.inference_mode():
                                            ...
                                            print_gpu_mem('Step 10 - Before loading sample')
                                            cloth_pure = sample["cloth_pure"].to(accelerator.device)
                                            print_gpu_mem('Step 11 - After loading sample')
                                            images = newpipe(
                ...
                # (chulayuth) text encoding of SD15 is a bit different from XL. there is the attentionmask, and CLIP_SKIP
                if hasattr(text_encoder.config, "use_attention_mask") and self.text_encoder.config.use_attention_mask:
                    attention_mask = text_inputs.attention_mask.to(device)
                else:
                    attention_mask = None

                if _CLIP_SKIP is None:
                    print_gpu_mem('Step 16 - Before loading text_input_ids')
                    encoder_output = text_encoder(text_input_ids.to(accelerator.device), attention_mask=attention_mask)
                    print_gpu_mem('Step 17 - After loading text_input_ids')
                    text_embeds = encoder_output[0]
                else:
                    print_gpu_mem('Step 16 - Before loading text_input_ids')
                    encoder_output = text_encoder(text_input_ids.to(accelerator.device), attention_mask=attention_mask, output_hidden_states=True)
                    print_gpu_mem('Step 17 - After loading text_input_ids')
                    text_embeds = encoder_output[-1][-(_CLIP_SKIP + 1)]
                    text_embeds = text_encoder.text_model.final_layer_norm(text_embeds)
                ...
                if _CLIP_SKIP is None:
                    ...
                    encoder_output = text_encoder(text_input_ids.to(accelerator.device), attention_mask=attention_mask)
                    print_gpu_mem('Step 17 - After loading text_input_ids')
                    text_embeds_cloth = encoder_output[0]
                else:
                    print_gpu_mem('Step 16 - Before loading text_input_ids')
                    encoder_output = text_encoder(text_input_ids.to(accelerator.device), attention_mask=attention_mask, output_hidden_states=True)
                    print_gpu_mem('Step 17 - After loading text_input_ids')
                    text_embeds_cloth = encoder_output[-1][-(_CLIP_SKIP + 1)]
                    text_embeds_cloth = text_encoder.text_model.final_layer_norm(text_embeds_cloth)

                _debug_print(f'>>>>>>>>> CALLING UNET_ENCODER: cloth_values.shape = {cloth_values.shape}')
                _debug_print(f'>>>>>>>>> CALLING UNET_ENCODER: timesteps = {timesteps}')
                _debug_print(f'>>>>>>>>> CALLING UNET_ENCODER: text_embeds_cloth.shape = {text_embeds_cloth.shape}')
