    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet), accelerator.accumulate(image_proj_model):
                ...
                if hasattr(encoder_output, 'text_embeds'):
                    # Case of 2nd text_embedding of SDXL, which often captures overall style.
                    _debug_print(f'>>>>>>>>>>>>>[2.X] encoder_output.text_embeds.shape = {encoder_output.text_embeds.shape}')
                    # We are only ALWAYS interested in the pooled output of the final text encoder
                    pooled_text_embeds = encoder_output[0]
                    ...
                    _debug_print(f'>>>>>>>>>>>>>[2.X] encoder_output.pooler_output.shape = {encoder_output.pooler_output.shape}')
                    pooled_text_embeds = encoder_output.pooler_output

                # encoder_output_2 = text_encoder(text_input_ids_2.to(accelerator.device), output_hidden_states=True)
